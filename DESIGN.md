...
## デザイン刷新 v2 — 「ヴィンテージ鉄道ポスター × 現代エディトリアル」

AI生成っぽさ(クリーム+金+丸角カード+絵文字チローム+グラデ頼み)を排し、
20世紀の仏伊旅行ポスターの気品と現代エディトリアルの余白で組み直す。
**クラス名・id・JSフックは一切変えない(CSSとマークアップの装飾部分のみ)。**

### トークン(style.css の :root を刷新)

- 紙白: --bg #f5f4ef / --surface #fdfcf9(クリーム味を抜く)、インク: --ink #171923
- --accent(ボイヤージュ・ブルー)#1d3f7a、--accent-2(ポスターレッド)#b7352b、
  --gold は真鍮 #97783d に落として使用箇所を1/3に(ホバー罫線・進捗のみ)
- --radius: 6px、影はほぼ廃止(--shadow は 0 1px 2px 程度)。カードは 1px ボーダー主体、
  ホバーは border-color 変化 + translateY(-1px)。ダークは同思想で夜牢紺 #101320 基調
- 罫線を階層表現の主役に: セクション頭に細罫、要素間は hairline

### チローム(飾り)の絵文字全廃

- タブ: 絵文字 → インラインSVG線画アイコン(stroke=currentColor, 1.5px, 20×20):
  home=コンパス / phrases=吹き出し / learn=開いた本 / culture=柱(パルテノン)/ quiz=的
- セクション見出し: 絵文字を外し「キッカー(仏/伊語の小さな大文字・字間0.14em・accent色)+日本語タイトル+細罫」
  例: PHRASE DU JOUR / 今日のフレーズ。ガイドの spot__subhead も同様(EN ROUTE / 順路 等)
- ボタン・チップ: 丸ピル → 角6pxの矩形。プライマリは accent ベタ+白、セカンダリはボーダー
- データ内の絵文字(カテゴリicon・文化記事icon等)は残すが、モノクロ調の小さな角バッジに収めて
  「素材」ではなく「刻印」に見せる(filter: grayscale(.2) 等はやり過ぎない範囲で)

### ヒーロー/カバー

- 夜明け/夕暮れのグラデーションは資産として維持。ただし内側に 1px の白罫フレーム(inset 14px)を足し、
  タイトルブロックを左寄せの組版に(eyebrow は大文字字間広め)。下端に「PARIS — VERSAILLES」の
  ルートライン(細罫+小さな駅名)を添えてポスターの体裁に
- ボタンはフレームに合わせ矩形・白ボーダー

### 適用範囲

- Opus A: css/style.css 全面 + css/learn.css の角丸/影の追随 + index.html / app-it.html のチローム
  (タブSVG・セクションキッカー・ヒーローフレーム)。quiz.js の注入スタイルはトークン継承で原則そのまま、
  丸ピルだけ矩形化が必要なら quiz.js 内の注入CSSを微修正可
- Opus B: css/guide.css + guide.html / guide-it.html(カバー枠・spot__subhead キッカー化・
  ストップカードのボーダー主体化・スポットナビのチップ矩形化)。データファイルは変更しない
- 検証: 4ページ×ライト/ダーク×390px のスクリーンショット全数確認。機能リグレッションなし
